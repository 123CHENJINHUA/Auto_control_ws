...
if __name__ == "__main__":
    ...
    # 初始化图形
    fig, ax = plt.subplots()
    ax.set_title("Real-time VR Data")
    ax.set_xlabel("Time (s)")
    ax.set_ylabel("Angle (degrees)")
    ax.set_xlim(0, 10)  # 显示最近 10 秒的数据
    ax.set_ylim(-0.15, 0.15)  # 调整纵轴范围为 -0.15 到 0.15
    roll_line, = ax.plot([], [], label="delta_roll", color="red")
    pitch_line, = ax.plot([], [], label="delta_pitch", color="green")
    yaw_line, = ax.plot([], [], label="delta_yaw", color="blue")
    ax.legend()

    def update(frame):
        act = ctrl.get_action()
        filtered_act = {k: v for k, v in act.items() if k in ["delta_roll", "delta_pitch", "delta_yaw"]}
        if any(filtered_act.values()):  # 检查是否有非零值
            current_time = time.time()
            time_data.append(current_time)
            roll_data.append(filtered_act["delta_roll"])
            pitch_data.append(filtered_act["delta_pitch"])
            yaw_data.append(filtered_act["delta_yaw"])

            # 保留最近 10 秒的数据
            if len(time_data) > 1 and time_data[-1] - time_data[0] > 10:
                time_data.pop(0)
                roll_data.pop(0)
                pitch_data.pop(0)
                yaw_data.pop(0)
