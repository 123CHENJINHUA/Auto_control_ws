...
# # 尝试导入 VR 控制器（可选）
# try:
#     from ...vr_oculus2_pkg.vr_oculus2 import OculusQuest3Controller, VRConfig  # 修改为 vr_oculus2
# except Exception:
...
# Removed: button_command_bridge integration
...
class FairinoCartRobot:
    ...
    def connect(self):
        ...
        try:
            ...
            try:
                # Try to reset errors and enable robot
                self.robot.ResetAllError()
                time.sleep(1.0)
                self.robot.RobotEnable(1) # Enable
                time.sleep(1.0)
                ...
                if ret != 0:
                    logger.warning(f"ServoMoveStart failed: {ret}")
            except Exception as e:
                logger.warning(f"Initialization commands failed for {self.robot_ip}: {e}")
    ...
    def _call_servo(self, mode: int, desc_pos: Sequence[float]):
        ...
        try:
            ...
            if ret != 0:
                logger.warning(f"ServoCart returned error: {ret} (mode={mode})")
        except Exception as e:
    ...
    def send_delta(self, delta6: Sequence[float]):
        ...
        if self._side == 'right':
             logger.info(f"Sending delta to right robot: {scaled_delta6}")
    ...
    def apply_action_dict(self, action: Dict[str, Any], mapping: str):
        ...
        if mapping == "absolute":
            ...
            # Expect unprefixed keys; DualFairinoCartRobot strips left_/right_ before passing in
            keys = ["delta_x", "delta_y", "delta_z", "delta_roll", "delta_pitch", "delta_yaw"]
            if all(k in action for k in keys):
    ...
    def get_tcp_pose(self) -> Optional[list]:
        ...
        try:
            ...
            # The SDK may return either (errcode, pose6), a bare pose list, or an error code.
            if isinstance(ret_org, (list, tuple)):
                # case: (errcode, pose6)
                if len(ret_org) >= 2 and isinstance(ret_org[1], (list, tuple)):
                    pose = list(ret_org[1])
                    return pose if len(pose) == 6 else None
                # case: bare pose6
                if len(ret_org) == 6 and all(isinstance(x, (int, float)) for x in ret_org):
                    return list(ret_org)
                return None
            if isinstance(ret_org, dict):
                pose = ret_org.get('pose') or ret_org.get('tcp')
                if isinstance(pose, (list, tuple)) and len(pose) == 6:
                    return list(pose)
                return None
            # If it's an int/err code or unexpected type, treat as unavailable
            return None
        except Exception as e:
            logger.debug(f"获取 TCP 位姿异常: {e}")
            return None
...
class DualFairinoCartRobot(Node):
    ...
    def __init__(self, config1: FairinoConfig, config2: FairinoConfig):
        ...
        # 默认配置（可由 main 赋值覆盖）
        self._mapping = 'delta_base'
        self._gripper_toggle_button = 'B'
        self._joint6_step = -5.0
        ...
        # 订阅 VR 动作话题（JSON 格式，与之前类似）
        self.vr_actions_sub = self.create_subscription(
            String,
            '/vr/actions',
    ...
    def actions_callback(self, msg: String):
        """接收 /vr/actions 的 JSON 字符串并应用到双臂和工具/夹爪逻辑"""
        try:
            data = msg.data or ''
            act: Dict[str, Any] = json.loads(data) if data else {}
        except Exception as e:
            self.get_logger().warning(f"Invalid /vr/actions JSON: {e}")
            return
        self._process_actions(act)

    def _process_actions(self, act: Dict[str, Any]) -> None:
        """Apply mapping, process buttons, bridge, gripper, and A-button logic."""
        # 机器人运动映射
        try:
            self.apply_action_dict(act, mapping=self._mapping, flag=True)
        except Exception:
            logger.exception('apply_action_dict failed')
        ...
        # gripper 接收开关：按下切换（默认 B 键）
        try:
            toggle_pressed = False
            btn = self._gripper_toggle_button
            if isinstance(status, dict) and btn in status:
                toggle_pressed = bool(status.get(btn, {}).get('pressed'))
            if toggle_pressed and not getattr(self, '_gripper_toggle_last', False):
                ...
                if not self._gripper_enabled:
                    self._gripper_pending_pct = None
                    self._gripper_pending_count = 0
                logger.info('Gripper receive toggled to %s (button=%s)', self._gripper_enabled, btn)
            self._gripper_toggle_last = toggle_pressed
        except Exception:
            logger.exception('Failed to process gripper toggle button')

        # Removed: forwarding button status to button_command_bridge
        ...
        try:
            ...
            logger.exception('Outer right-motion handling failed')

        # A 键：上升沿时让右臂关节6旋转一个步长
        try:
            a_pressed = False
    ...
    def apply_action_dict(self, action: Dict[str, Any], mapping: str, flag):
        ...
        # self.get_logger().info(f"rightGrip: {status.get('rightGrip')}")
...
def main():
    ...
    # Allow ROS 2 launch to pass --ros-args etc without breaking argparse
    ...
    # 设置处理参数以供 /vr/actions 使用
    dual_robot._mapping = args.mapping
    dual_robot._joint6_step = args.joint6_step
    dual_robot._gripper_toggle_button = 'B'

    # Removed: button_command_bridge node setup
    ...
    # Spin ROS2 nodes via a shared executor
    executor = MultiThreadedExecutor()
    executor.add_node(dual_robot)
    # Removed: adding button_command_bridge to executor
    exec_thread = threading.Thread(target=executor.spin, daemon=True)
    exec_thread.start()

    # Now connect and listen to /vr/actions
    dual_robot.connect()
    try:
        # 主线程保持运行，ROS2 在内部线程自旋并处理 /vr/actions
        while True:
            time.sleep(0.5)
    except KeyboardInterrupt:
        ...
        # Removed: destroying button_command_bridge node
        rclpy.shutdown()
