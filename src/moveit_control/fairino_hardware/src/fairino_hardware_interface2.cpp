#include "fairino_hardware/fairino_hardware_interface2.hpp"

namespace fairino_hardware{

hardware_interface::CallbackReturn FairinoHardwareInterface2::on_init(const hardware_interface::HardwareInfo& sysinfo){
...
std::vector<hardware_interface::StateInterface> FairinoHardwareInterface2::export_state_interfaces()
{
...
std::vector<hardware_interface::CommandInterface> FairinoHardwareInterface2::export_command_interfaces()
{
...
hardware_interface::CallbackReturn FairinoHardwareInterface2::on_activate(const rclcpp_lifecycle::State& previous_state)
{
    using namespace std::chrono_literals;
    RCLCPP_INFO(rclcpp::get_logger("FairinoHardwareInterface2"), "Starting ...please wait...");
    //做变量的初始化工作
    ...
    if(returncode != 0){
        RCLCPP_INFO(rclcpp::get_logger("FairinoHardwareInterface2"), "机械臂SDK连接失败！请检查端口时候被占用");
        return hardware_interface::CallbackReturn::ERROR;
    }else{
        RCLCPP_INFO(rclcpp::get_logger("FairinoHardwareInterface2"), "机械臂SDK连接成功！");
    }
    //做第一步的工作，读取当前状态数据
    JointPos jntpos;
    ...
    */
    if(returncode == 0){
        ...
        RCLCPP_INFO(rclcpp::get_logger("FairinoHardwareInterface2"),"初始指令位置: %f,%f,%f,%f,%f,%f",_jnt_position_command[0],\
        _jnt_position_command[1],_jnt_position_command[2],_jnt_position_command[3],_jnt_position_command[4],_jnt_position_command[5]);    
        RCLCPP_INFO(rclcpp::get_logger("FairinoHardwareInterface2"), "机械臂硬件启动成功!");
        return hardware_interface::CallbackReturn::SUCCESS;
    }else{
        RCLCPP_INFO(rclcpp::get_logger("FairinoHardwareInterface2"), "读取初始关节角度错误，硬件无法启动！请检查通讯内容");
        return hardware_interface::CallbackReturn::ERROR;
    }
}
...
hardware_interface::CallbackReturn FairinoHardwareInterface2::on_deactivate(const rclcpp_lifecycle::State& previous_state)
{
    RCLCPP_INFO(rclcpp::get_logger("FairinoHardwareInterface2"), "Stopping ...please wait...");
    _ptr_robot->StopMotion();//停止机器人
    _ptr_robot->CloseRPC();//销毁实例，连接断开
    _ptr_robot.release();
    RCLCPP_INFO(rclcpp::get_logger("FairinoHardwareInterface2"), "System successfully stopped!");
    return hardware_interface::CallbackReturn::SUCCESS;
}



hardware_interface::return_type FairinoHardwareInterface2::read(const rclcpp::Time& time,const rclcpp::Duration& period)
...
hardware_interface::return_type FairinoHardwareInterface2::write(const rclcpp::Time& time,const rclcpp::Duration& period)
{
    if(_control_mode == 0){//位置控制模式
        ...
        for(auto j=0;j<6;j++){
            ...
            // cmd.jPos[j] = state_data.jPos[j] + 0.05;
        }
        ...
        RCLCPP_INFO(rclcpp::get_logger("FairinoHardwareInterface2"), "指令发送错误:未识别当前所处控制模式");
        return hardware_interface::return_type::ERROR;
    }
...
PLUGINLIB_EXPORT_CLASS(fairino_hardware::FairinoHardwareInterface2, hardware_interface::SystemInterface)
