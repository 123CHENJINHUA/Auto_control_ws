...
def generate_launch_description():
    ...
    urdf_file_names = ['my_robot_16.xacro','my_robot_5.xacro']  # 替换为你的URDF文件名
    ...
    for i in range(2):
        ...
        action_declare_arg_model_path = launch.actions.DeclareLaunchArgument(
            name='model%d'%(i), default_value=str(default_urdf_file_path),description='URDF file'
        )

        substitutions_command_result = launch.substitutions.Command(['xacro ', launch.substitutions.LaunchConfiguration('model%d'%(i))])
        ...
        action_spawn_entity = launch_ros.actions.Node(
            ...
            name='spawn_entity',
            namespace=name_space[i],
            arguments=['-topic', 'robot_description', '-entity', os.path.splitext(urdf_file_names[i])[0]],
            output='screen'
        )
        
        # load the real controller
        action_load_controller = launch_ros.actions.Node(
        ...
        goal_list.append(action_load_controller)
